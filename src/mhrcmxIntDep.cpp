...
// // header file of truncated sigma sample
// #include "rsig_ars_add.h"

// header file of theta sampler
#include "update_theta_integrated_dep.h"
...
// fbeta_log_int_dep
double fbeta_log_int_dep(const arma::mat& Yi, const arma::mat& WHi, const arma::mat& WHHi,
                         const arma::rowvec beta0i, const arma::mat Wi_inv,
                         const arma::mat theta, arma::rowvec lam2,
                         const int i, const int D, const int P, const arma::mat& Ip) {
  
  int ni = Yi.n_rows;
  arma::vec col1ni = arma::ones(ni);
  arma::vec yi = (Yi - col1ni*beta0i).as_col();
  // arma::mat WHi = kron(Wi_inv,Hi.t());
  // arma::mat WHHi = kron(Wi_inv,HHi);
  arma::mat lj_inv(D,D, arma::fill::zeros);
  for (int d = 0; d < D; d++) lj_inv(d,d) = 1/lam2(d);
  arma::mat Lj_inv = kron(lj_inv,Ip);

  arma::mat Vi_inv = WHHi + Lj_inv;
  // arma::mat Vi = arma::inv(WHHi + Lj_inv);
  arma::mat Vi = inv_sympd(WHHi + Lj_inv);
  // arma::vec mi = WHi*yi + Lj_inv*theta.as_col();
  arma::vec mi = WHi*yi + (theta*lj_inv).as_col();
  
  double out = - double(P)*sum(log(lam2)) - log(det(Vi_inv));
  out -= arma::trace(theta.t()*theta*lj_inv);
  out += arma::as_scalar( mi.t()*Vi*mi );
...
////* hrcmx *////
// [[Rcpp::export]]
Rcpp::List mhrcmx_dep_rep_int(const Rcpp::List Y,        // observations
                  ...
                  const double Asig=0,                  // sig2 prior (truncated)
                  const double dof=1,                   // degrees of freedom IW
                  const double eta=1,                   // LKJ parameter
                  const double eta0=1,                  // LKJ proposal parameter
  ...
  W.each_slice() = ID;
  R.each_slice() = ID;
  const arma::mat Psi = ID;
  arma::cube Winv(D,D,m);
  Winv.each_slice() = ID;
  arma::field<arma::mat> WH(m);
  int PD = P*D;
  arma::cube WHH(PD,PD,m);
  
  for (int i = 0; i < m; i++) {
    arma::mat Hi = H[i];
    WH[i] = kron(Winv.slice(i),Hi.t());
    WHH.slice(i) = kron(Winv.slice(i),HH.slice(i));
  }
  ...
  arma::mat accept_logratio(J,ns, arma::fill::zeros);
  arma::vec accept_logratio_vec(J, arma::fill::zeros);
  arma::Mat<int> accept(J,ns, arma::fill::zeros);
  arma::Col<int> accept_vec(J, arma::fill::zeros);
  
  
  
  // LKJ sampler
  arma::Row<int> accept_lkj(m, arma::fill::zeros);
  arma::Mat<int> accept_lkj_chain(ns,m, arma::fill::zeros);
  ...
  arma::Mat<int> accept_mat(J, nadapt, arma::fill::zeros);
  double epsD = eps;
  double eps0D = eps0;
  ...
  arma::field<arma::cube> W_chain(ns1);
  arma::field<arma::cube> R_chain(ns1);
  ...
  while (s < ns) {
    ...
    if (ppmx == 1) {
      ...
      for (int i = 0; i < m; i++) {
        ...
        arma::mat Yi = Y[i];
        // arma::mat Wi_inv = inv_sympd(W.slice(i));
        arma::mat Wi_inv = Winv.slice(i);
        arma::mat WHi = WH[i];
        arma::mat WHHi = WHH.slice(i);
        ...
        if (nj(zi) > 0) {
          ...
          for (int j = 0; j < m_neal; j++) {
            ...
            // }
            fbeta_log(nc+j) += fbeta_log_int_dep(Yi, WHi, WHHi, beta0.row(i), Wi_inv, theta_new.col_as_mat(j), lam2_new.row(j), i, D, P, Ip);
          ...
          for (int j = 0; j < nc; j++) {
            ...
            // }
            fbeta_log(j) += fbeta_log_int_dep(Yi, WHi, WHHi, beta0.row(i), Wi_inv, theta.col_as_mat(j), lam2.row(j), i, D, P, Ip);
        ...
        if (nj(zi) == 0) {
          ...
          for (int j = 0; j < nc; j++) {
            ...
            // }
            fbeta_log(j) += fbeta_log_int_dep(Yi, WHi, WHHi, beta0.row(i), Wi_inv, theta.col_as_mat(h), lam2.row(h), i, D, P, Ip);
    ...
    if (fmmx == 1) {
      ...
      if (covariates_fmm == 1) { // LSBP strategy (Rigon etal 2021)
        ...
        for (int i = 0; i < m; i++) {
          ...
          // if (b <= burn) {
          //   Rprintf("\n");
          //   Rprintf("W%i_inv(0,0)=%f\n", i,Wi_inv(0,0));
          //   Rprintf("W%i_inv(0,1)=%f\n", i,Wi_inv(0,1));
          //   Rprintf("W%i_inv(1,0)=%f\n", i,Wi_inv(1,0));
          //   Rprintf("W%i_inv(1,1)=%f\n", i,Wi_inv(1,1));
          // }
          ...
          for (int j = 0; j < J; j++) {
            ...
            // beta integrated out (dependent model)
            fbeta_log(j) += fbeta_log_int_dep(Yi, WHi, WHHi, beta0.row(i), Wi_inv, theta.col_as_mat(j), lam2.row(j), i, D, P, Ip);
            
            // Rprintf("\n");
            // Rprintf("lam2(%i,0)=%f\n", j,lam2(j,0));
            // Rprintf("lam2(%i,1)=%f\n", j,lam2(j,1));
            // Rprintf("fbeta_log(%i)=%f\n", j,fbeta_log(j));
    ...
    update_theta_dep(theta, Winv, WH, WHH, accept_vec, accept_logratio_vec,
                     nj, lam2, tau2, beta0, sig2, mu, z, K, Ki,
                     m, D, J, P, PD, Ip, Y, n_theta, Ht0, theta_sampler,
                     repulsive, phi, nu, r, nr, epsD, eps0D);
    ...
    if (repulsive != 0) {
      if (b <= burn) {
        ...
        if (b % nadapt == 0) {
          ...
          arma::Col<int> acc(J, arma::fill::zeros);
          for (int n = 0; n < nadapt; n++) acc += accept_mat.col(n);
          
          arma::uvec jn = arma::find(nj>0);
          arma::uvec j0 = arma::find(nj==0);
          
          // eps update
          double rmin = arma::min( acc.elem(jn) )/(double)nadapt;
          double rmax = arma::max( acc.elem(jn) )/(double)nadapt;
          if (rmin > 0.50) {
            epsD *= 1.20;
          } else if (rmin < 0.05) {
            epsD *= 0.95;
          ...
          // eps0 update
          double rmin0 = arma::min( acc.elem(j0) )/(double)nadapt;
          double rmax0 = arma::max( acc.elem(j0) )/(double)nadapt;
          if (rmin0 > 0.50) {
            eps0D *= 1.20;
          } else if (rmin0 < 0.01) {
            eps0D *= 0.95;
          ...
          accept_mat.col(beps) = accept_vec;
          beps += 1;
    ...
    for (int i = 0; i < m; i++) {
      ...
      int ni = Yi.n_rows;
      arma::vec col1ni = arma::ones(ni);
      arma::vec yi = (Yi - col1ni*beta0.row(i)).as_col();
      // arma::mat Wi_inv = arma::inv(W.slice(i));
      // arma::mat Wi_inv = inv_sympd(W.slice(i));
      // arma::mat Wi_inv = Winv.slice(i);
      arma::mat lj_inv(D,D, arma::fill::zeros);
      for (int d = 0; d < D; d++) lj_inv(d,d) = 1/lam2(zi,d);
      ...
      arma::mat Lj_inv = kron(lj_inv,Ip);
      arma::mat theta_zi(P,D);
      for (int d = 0; d < D; d++) {
        for (int p = 0; p < P; p++) {
          theta_zi(p,d) = theta(p,zi,d);
        }
      }
      
      // arma::mat V_beta = arma::inv(WHH + Lj_inv);
      arma::mat V_beta = inv_sympd(WHHi + Lj_inv);
      arma::vec m_beta = V_beta * (WHi*yi + Lj_inv*theta_zi.as_col());
      arma::vec beta_vec = rmvnorm(1, m_beta , V_beta).t();
      int pos = -1;
      for (int d = 0; d < D; d++) {
        for (int p = 0; p < P; p++) {
          pos += 1;
          beta(p,i,d) = beta_vec(pos);
        }
      }
    ...
    if (mu_par == 1) {
      for (int d = 0; d < D; d++) {
        ...
        arma::mat V_mu = inv_sympd( K*sum_tau2 + Ip/s2mu );
    ...
    for (int i = 0; i < m; i++) {
      ...
      // arma::mat Wi_inv = inv_sympd(W.slice(i));
      arma::mat Wi_inv = Winv.slice(i);
      arma::vec sum_yi(D, arma::fill::zeros);
      arma::mat sig02inv(D,D, arma::fill::zeros);
      for (int d = 0; d < D; d++) {
        sum_yi(d) = sum(Yi.col(d));
        sig02inv(d,d) = 1/sig02(d);
      }
      // arma::mat V_beta0 = arma::inv((double)ni*Wi_inv + sig02inv);
      arma::mat V_beta0 = inv_sympd((double)ni*Wi_inv + sig02inv);
      arma::vec m_beta0 = V_beta0 * (Wi_inv*sum_yi + sig02inv*mu0);
      beta0.row(i) = rmvnorm(1, m_beta0 , V_beta0);
      // Rprintf("beta0(%i) , b=%i\n", i,b);
      ...
      // W sample
      arma::vec col1ni = arma::ones(ni);
      arma::mat Ei = Yi - Hi*beta.col_as_mat(i) - col1ni*beta0.row(i);
      arma::mat Ei2 = Ei.t()*Ei;
      
      // if (Asig == 0) {
        
        W.slice(i) = riwish( dof + double(ni) , Ei2 + Psi );
        sig2.row(i) = W.slice(i).diag().t();
        Winv.slice(i)= inv_sympd(W.slice(i));

      // } else {
      // 
      //   arma::mat Ri_inv = R.slice(i).i();
      //   
      //   // S sample
      //   for (int d = 0; d < D; d++) {
      //     double a_sig = Ri_inv(d,d) * Ei2(d,d);
      //     double b_sig = 0;
      //     for (int l = 0; l < D; l++) {
      //       if (l != d) b_sig += 1/sqrt(sig2(i,l)) * Ri_inv(d,l) * Ei2(d,l);
      //     }
      //     // ARS to sample from w = 1/sig
      //     double sig = 1/rsig_ars(Asig, Lu, ni, a_sig, b_sig);
      //     S(d,d,i) = sig;
      //     sig2(i,d) = sig*sig;
      //   }
      //   
      ...
      //   // R sample
      //   unsigned int seed;
      //   Eigen::MatrixXd Ri_prop(D,D);
      //   double detRi_prop = 0;
      //   // int ndet0 = 0;
      //   
      //   while (detRi_prop <= 0) {
      //     seed = Rcpp::sample(max_int,1)[0];
      //     Ri_prop = rlkjcorr_cpp(D, eta0, seed);
      //     detRi_prop = Ri_prop.determinant();
      //     // if (detRi_prop <= 0) {
      //     //   ndet0 += 1;
      //     //   Rprintf("count = %i , seed = %i\n", ndet0,seed);
      //     // }
      //   }
      // 
      //   arma::mat Si = S.slice(i);
      //   arma::mat Si_inv(D,D, arma::fill::zeros);
      //   for (int d = 0; d < D; d++) Si_inv(d,d) = 1/Si(d,d);
      //   arma::mat SEES = Si_inv * Ei2 * Si_inv;
      //   double detRi = arma::det(R.slice(i));
      //   
      //   Eigen::MatrixXd Ri_prop_inv = Ri_prop.inverse();
      //   arma::mat Ri_prop_inv_arma = arma::mat(Ri_prop_inv.data(),
      //                                          Ri_prop_inv.rows(),
      //                                          Ri_prop_inv.cols(),
      //                                          false, false);
      //   
      //   double log_ratio_lkj = (-(double)ni/2+eta-eta0)*(log(detRi_prop)-log(detRi)) -
      //     0.5*arma::trace((Ri_prop_inv_arma-Ri_inv)*SEES);
      //   
      //   double u = Rf_runif(0.0,1.0);
      //   if (log(u) < log_ratio_lkj) {
      //     R.slice(i) = arma::mat(Ri_prop.data(),
      //             Ri_prop.rows(),
      //             Ri_prop.cols(),
      //             false, false);
      //     accept_lkj(i) = 1;
    ...
    if (b > burn) {
      if ((b-burn) % thin == 0) {
        ...
        if (only_nclr == 0) {
          ...
          W_chain(s) = W;
          // if (Asig != 0) R_chain(s) = R;
          // if (Asig != 0) accept_lkj_chain.row(s) = accept_lkj;
        ...
        // acceptance
        if (repulsive != 0) {
          accept_logratio.col(s) = accept_logratio_vec;
          accept.col(s) = accept_vec;
        }
  ...
  if (only_nclr == 0) {
    ...
    // if (Asig != 0) {
    //   out["R"] = R_chain;
    //   out["accept_lkj"] = accept_lkj_chain;
    // }
