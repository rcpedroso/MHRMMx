...
// header file of some functions in SM
#include "SM_auxiliary.h"
#include "SM_auxiliary_theta.h"
...
// likelihood_log_int
double likelihood_log_int(const arma::mat& Hi, const arma::mat& HHi, const arma::mat& Yi,
                          const arma::mat beta0, const arma::mat sig2,
                          const arma::vec theta, const double lam2,
                          const int i, const int d, const int P, const arma::mat& Ip,
                          const int ni, const double log2pi) {
  ...
  // additional individual-specific values
  out-= (double)ni*log2pi;
  out-= (double)ni*log(sig2(i,d));
...
////* hrcmx *////
// [[Rcpp::export]]
Rcpp::List mhrcmx_ind_rep_int_SM(const Rcpp::List Y,    // observations
  ...
  // constants
  const double a_tau2 = at + 0.5*(double)P;
    ...
    if (SM == 1) {
      ...
      while (t < nGS) {
        ...
        for (int d = 0; d < D; d++) {
          ...
          // arma::mat beta_s = beta_SM_s(beta, z_split, zij, i, j, S, nS, d, P);
          // for (int l = 0; l < 2; l++) {
          //   arma::mat Vi_s = K/tau2_s(l,d) + (double)nj_s(l)*Ip/lam2_s(l,d);
          //   arma::mat V_s = arma::inv( Vi_s );
          //   arma::vec m_s = V_s * ( K*mu.col(d)/tau2_s(l,d) + beta_s.col(l)/lam2_s(l,d) );
          //   theta_s.slice(d).col(l) = rmvnorm(1, m_s , V_s).t();
          // }

          // Rcpp::List sumV = sumV_SM_s(lam2_s, z_split, zij, i, j, S, nS, beta0, sig2, Ip, d, P, Y, H, HH);
          // arma::cube sumV_s = sumV["sumV"];
          // arma::mat sumVH_s = sumV["sumVH"];
          // for (int l = 0; l < 2; l++) {
          //   arma::mat Vi_s = lam2_s(l,d)*K/tau2_s(l,d) + (double)nj_s(l)*Ip - sumV_s.slice(l);
          //   arma::mat V_s = arma::inv( Vi_s ) * lam2_s(l,d);
          //   arma::vec m_s = V_s * ( K*mu.col(d)/tau2_s(l,d) + sumVH_s.col(l) );
          //   theta_s.slice(d).col(l) = rmvnorm_eigen(m_s , V_s, lam2_s(l,d), P).t();
          // }
          ...
          arma::mat sumVH_s = sumV["sumVH"];
          for (int l = 0; l < 2; l++) {
            arma::mat Vi_s = K/tau2_s(l,d) + (double)nj_s(l)/lam2_s(l,d)*Ip - sumV_s.slice(l)/lam2_s(l,d)/lam2_s(l,d);
            arma::mat V_s = arma::inv( Vi_s );
            arma::vec m_s = V_s * ( K*mu.col(d)/tau2_s(l,d) + sumVH_s.col(l)/lam2_s(l,d) );
            theta_s.slice(d).col(l) = rmvnorm(1, m_s , V_s).t();;
          }
          ...
          if (ppmx==1) {
            for (int l = 0; l < 2; l++) {
              double b_tau2_s = b_tau2_SM(theta_s.col_as_mat(l), mu, btD, K, d);
              tau2_s(l,d) = 1/Rf_rgamma( a_tau2 , 1/b_tau2_s );
            }
          }
          
          
          /* lam2_s sample (ARS to sample from w = 1/x) */
          arma::vec beta_theta_s = beta_theta_SM_s(theta_s, beta, z_split, zij, i, j, S, nS, d, 0, 1);
          for (int l = 0; l < 2; l++) {
            int njP = nj_s(l)*P;
            double a_lam2_s = 0.5*((double)njP-1);
            double b_lam2_s = 0.5*beta_theta_s(l);
        ...
        for (int k = 0; k < nS; k++) {
          ...
          // data weights
          arma::mat Hk = H[S(k)];
          arma::mat HHk = HH.slice(S(k));
          arma::mat Yk = Y[S(k)];
          for (int l = 0; l < 2; l++) {
            for (int d = 0; d < D; d++) {
              // logPij(l) += dmvnorm_log(beta.slice(d).col(S(k)), theta_s.slice(d).col(l), lam2_s(l,d), P);
              logPij(l) += fbeta_log_int(Hk, HHk, Yk, beta0, sig2, theta_s.slice(d).col(l), lam2_s(l,d), S(k), d, P, Ip);
            }
          }
          

          // z_split sample
          if (logPij(0) == logPij(1)) {
            Pij(0,k) = 0.5;
            Pij(1,k) = 0.5;
          } else {
      ...
      while (t < nGS) {
        ...
        for (int d = 0; d < D; d++) {
          ...
          /* tau2_m sample */
          if (ppmx==1) {
            double b_tau2_m = b_tau2_SM(theta_m, mu, btD, K, d);
            tau2_m(d) = 1/Rf_rgamma( a_tau2 , 1/b_tau2_m );
          }
          
          /* theta_m sample */
          
          // arma::vec beta_m = beta_SM_m(beta, i, j, S, nS, d);
          // arma::mat Vi_m = K/tau2_m(d) + (double)nj_m*Ip/lam2_m(d);
          // arma::mat V_m = arma::inv( Vi_m );
          // arma::vec m_m = V_m * ( K*mu.col(d)/tau2_m(d) + beta_m/lam2_m(d) );
          // theta_m.col(d) = rmvnorm(1, m_m , V_m).t();
          
          // Rcpp::List sumV = sumV_SM_m(lam2_m, i, j, S, nS, beta0, sig2, Ip, d, P, Y, H, HH);
          // arma::mat sumV_m = sumV["sumV"];
          // arma::vec sumVH_m = sumV["sumVH"];
          // arma::mat Vi_m = lam2_m(d)*K/tau2_m(d) + (double)nj_m*Ip - sumV_m;
          // arma::mat V_m = arma::inv( Vi_m ) * lam2_m(d);
          // arma::vec m_m = V_m * ( K*mu.col(d)/tau2_m(d) + sumVH_m );
          // theta_m.col(d) = rmvnorm_eigen(m_m , V_m, lam2_m(d), P).t();

          Rcpp::List sumV = sumV_SM_m(lam2_m, i, j, S, nS, beta0, sig2, Ip, d, P, Y, H, HH);
          arma::mat sumV_m = sumV["sumV"];
          arma::vec sumVH_m = sumV["sumVH"];
          arma::mat Vi_m = K/tau2_m(d) + (double)nj_m/lam2_m(d)*Ip - sumV_m/lam2_m(d)/lam2_m(d);
          arma::mat V_m = arma::inv( Vi_m );
          arma::vec m_m = V_m * ( K*mu.col(d)/tau2_m(d) + sumVH_m/lam2_m(d) );
          theta_m.col(d) = rmvnorm(1, m_m , V_m).t();
          
                    
          /* lam2_m sample (ARS to sample from w = 1/x) */
          double beta_theta_m = beta_theta_SM_m(theta_m, beta, i, j, S, nS, d);
          int njP = nj_m*P;
          double a_lam2 = 0.5*((double)njP-1);
          double b_lam2 = 0.5*beta_theta_m;
      ...
      if (zi == zj) {
        ...
        for (int d = 0; d < D; d++) {
          ...
          if (ppmx==1) {
            for (int l = 0; l < 2; l++) {
              double b_tau2_s = b_tau2_SM(theta_s.col_as_mat(l), mu, btD, K, d);
              tau2_s(l,d) = 1/Rf_rgamma( a_tau2 , 1/b_tau2_s );
              log_phi_s4 += dinvgamma_log(tau2_s(l,d), a_tau2, b_tau2_s);
            }
          }
          ...
          for (int l = 0; l < 2; l++) {
            int njP = nj_s(l)*P;
            double a_lam2 = 0.5*((double)njP-1);
            double b_lam2 = 0.5*beta_theta_s(l);
        ...
        for (int d = 0; d < D; d++) {
          ...
          if (ppmx==1) {
            ...
            double b_tau2_m = b_tau2_SM(theta_m, mu, btD, K, d);
            // tau2_m(d) = 1/Rf_rgamma( a_tau2 , 1/b_tau2_m ); // HYPOTETICAL Gibbs sampling
            log_phi_m4 += dinvgamma_log(tau2(zi,d), a_tau2, b_tau2_m);
          }
          ...
          // arma::mat V_m = arma::inv( Vi_m );
          // arma::vec m_m = V_m * ( K*mu.col(d)/tau2_m(d) + beta_m/lam2_m(d) );
          // // theta_m.col(d) = rmvnorm(1, m_m , V_m).t(); // HYPOTETICAL Gibbs sampling
          // log_phi_m4 += dmvnorm(theta.slice(d).col(zi).t(), m_m, V_m, true).at(0);

          // Rcpp::List sumV = sumV_SM_m(lam2_m, i, j, S, nS, beta0, sig2, Ip, d, P, Y, H, HH);
          // arma::mat sumV_m = sumV["sumV"];
          // arma::vec sumVH_m = sumV["sumVH"];
          // arma::mat Vi_m = lam2_m(d)*K/tau2(zi,d) + (double)nj_m*Ip - sumV_m;
          // arma::mat V_m = arma::inv( Vi_m ) * lam2_m(d);
          // arma::vec m_m = V_m * ( K*mu.col(d)/tau2(zi,d) + sumVH_m );
          // // theta_m.col(d) = rmvnorm_eigen(m_m , V_m, lam2_m(d), P).t(); // HYPOTETICAL Gibbs sampling
          // log_phi_m4 += dmvnorm(theta.slice(d).col(zi).t(), m_m, V_m, true).at(0);
          
          Rcpp::List sumV = sumV_SM_m(lam2_m, i, j, S, nS, beta0, sig2, Ip, d, P, Y, H, HH);
          arma::mat sumV_m = sumV["sumV"];
          arma::vec sumVH_m = sumV["sumVH"];
          arma::mat Vi_m = K/tau2(zi,d) + (double)nj_m/lam2_m(d)*Ip - sumV_m/lam2_m(d)/lam2_m(d);
          arma::mat V_m = arma::inv( Vi_m );
          arma::vec m_m = V_m * ( K*mu.col(d)/tau2(zi,d) + sumVH_m/lam2_m(d) );
          // theta_m.col(d) = rmvnorm(1, m_m , V_m).t(); // HYPOTETICAL Gibbs sampling
          log_phi_m4 += dmvnorm(theta.slice(d).col(zi).t(), m_m, V_m, true).at(0);
          ...
          double beta_theta_m = beta_theta_SM_m(theta.col_as_mat(zi), beta, i, j, S, nS, d);
          int njP = nj_m*P;
          double a_lam2 = 0.5*((double)njP-1);
          double b_lam2 = 0.5*beta_theta_m;
        ...
        for (int d = 0; d < D; d++) {
          ...
          log_L += likelihood_log_int(Hi, HHi, Yi, beta0, sig2, theta_s.slice(d).col(0), lam2_s(0,d), i, d, P, Ip, ni, log2pi);
          log_L += likelihood_log_int(Hj, HHj, Yj, beta0, sig2, theta_s.slice(d).col(1), lam2_s(1,d), j, d, P, Ip, nj, log2pi);
          log_L -= likelihood_log_int(Hi, HHi, Yi, beta0, sig2, theta.slice(d).col(zi), lam2(zi,d), i, d, P, Ip, ni, log2pi); // zi == zj
          log_L -= likelihood_log_int(Hj, HHj, Yj, beta0, sig2, theta.slice(d).col(zj), lam2(zj,d), j, d, P, Ip, nj, log2pi); // zi == zj
          ...
          for (int k = 0; k < nS; k++) {
            ...
            if (z_split(S(k))== zij(0)) {
              log_L += likelihood_log_int(Hk, HHk, Yk, beta0, sig2, theta_s.slice(d).col(0), lam2_s(0,d), S(k), d, P, Ip, nk, log2pi);
              log_L -= likelihood_log_int(Hk, HHk, Yk, beta0, sig2, theta.slice(d).col(zi), lam2(zi,d), S(k), d, P, Ip, nk, log2pi); // zi == zj
            } else {
              log_L += likelihood_log_int(Hk, HHk, Yk, beta0, sig2, theta_s.slice(d).col(1), lam2_s(1,d), S(k), d, P, Ip, nk, log2pi);
              log_L -= likelihood_log_int(Hk, HHk, Yk, beta0, sig2, theta.slice(d).col(zj), lam2(zj,d), S(k), d, P, Ip, nk, log2pi); // zi == zj
            }
          }
        }
      ...
      if (zi != zj) {
        ...
        for (int d = 0; d < D; d++) {
          ...
          if (ppmx==1) {
            double b_tau2_m = b_tau2_SM(theta_m, mu, btD, K, d);
            tau2_m(d) = 1/Rf_rgamma( a_tau2 , 1/b_tau2_m );
            log_phi_m5 += dinvgamma_log(tau2_m(d), a_tau2, b_tau2_m);
          }
        ...
        for (int d = 0; d < D; d++) {
          ...
          if (ppmx==1) {
            for (int l = 0; l < 2; l++) {
              double b_tau2_s = b_tau2_SM(theta_s.col_as_mat(l), mu, btD, K, d);
              if (l == 0) {
                // tau2_s(l,d) = 1/Rf_rgamma( a_tau2 , 1/b_tau2_s ); // HYPOTETICAL Gibbs sampling
                log_phi_s5 += dinvgamma_log(tau2(zi,d), a_tau2, b_tau2_s);
              } else {
                // tau2_s(l,d) = 1/Rf_rgamma( a_tau2 , 1/b_tau2_s ); // HYPOTETICAL Gibbs sampling
                log_phi_s5 += dinvgamma_log(tau2(zj,d), a_tau2, b_tau2_s);
              }
            }
          }
          ...
          // for (int l = 0; l < 2; l++) {
          //   int zl = zij(l);
          //   arma::mat Vi_s = K/tau2(zl,d) + (double)nj_s(l)*Ip/lam2_s(l,d);
          //   arma::mat V_s = arma::inv( Vi_s );
          //   arma::vec m_s = V_s * ( K*mu.col(d)/tau2(zl,d) + beta_s.col(l)/lam2_s(l,d) );
          //   // theta_s.slice(d).col(l) = rmvnorm(1, m_s , V_s).t(); // HYPOTETICAL Gibbs sampling
          //   log_phi_s5 += dmvnorm(theta.slice(d).col(zl).t(), m_s, V_s, true).at(0);
          // }
          ...
          // for (int l = 0; l < 2; l++) {
          //   int zl = zij(l);
          //   arma::mat Vi_s = lam2_s(l,d)*K/tau2(zl,d) + (double)nj_s(l)*Ip - sumV_s.slice(l);
          //   arma::mat V_s = arma::inv( Vi_s ) * lam2_s(l,d);
          //   arma::vec m_s = V_s * ( K*mu.col(d)/tau2(zl,d) + sumVH_s.col(l) );
          //   // theta_s.slice(d).col(l) = rmvnorm_eigen(m_s , V_s, lam2_s(l,d), P).t(); // HYPOTETICAL Gibbs sampling
          //   log_phi_s5 += dmvnorm(theta.slice(d).col(zl).t(), m_s, V_s, true).at(0);
          // }
          ...
          for (int l = 0; l < 2; l++) {
            int zl = zij(l);
            arma::mat Vi_s = K/tau2(zl,d) + (double)nj_s(l)/lam2_s(l,d)*Ip - sumV_s.slice(l)/lam2_s(l,d)/lam2_s(l,d);
            arma::mat V_s = arma::inv( Vi_s );
            arma::vec m_s = V_s * ( K*mu.col(d)/tau2(zl,d) + sumVH_s.col(l)/lam2_s(l,d) );
            // theta_s.slice(d).col(l) = rmvnorm(1, m_s , V_s).t(); // HYPOTETICAL Gibbs sampling
            log_phi_s5 += dmvnorm(theta.slice(d).col(zl).t(), m_s, V_s, true).at(0);
          }
          ...
          /* (ARS to sample from w = 1/x) */
          arma::vec beta_theta_s = beta_theta_SM_s(theta, beta, z_split, zij, i, j, S, nS, d, zi, zj);
          for (int l = 0; l < 2; l++) {
            int njP = nj_s(l)*P;
            double a_lam2 = 0.5*((double)njP-1);
            double b_lam2 = 0.5*beta_theta_s(l);
        ...
        for (int d = 0; d < D; d++) {
          ...
          log_L += likelihood_log_int(Hi, HHi, Yi, beta0, sig2, theta_m.col(d), lam2_m(d), i, d, P, Ip, ni, log2pi);
          log_L += likelihood_log_int(Hj, HHj, Yj, beta0, sig2, theta_m.col(d), lam2_m(d), j, d, P, Ip, nj, log2pi);
          log_L -= likelihood_log_int(Hi, HHi, Yi, beta0, sig2, theta.slice(d).col(zi), lam2(zi,d), i, d, P, Ip, ni, log2pi);
          log_L -= likelihood_log_int(Hj, HHj, Yj, beta0, sig2, theta.slice(d).col(zj), lam2(zj,d), j, d, P, Ip, nj, log2pi);
          ...
          for (int k = 0; k < nS; k++) {
            ...
            log_L += likelihood_log_int(Hk, HHk, Yk, beta0, sig2, theta_m.col(d), lam2_m(d), S(k), d, P, Ip, nk, log2pi);
            if (z(S(k))== zi) {
              log_L -= likelihood_log_int(Hk, HHk, Yk, beta0, sig2, theta.slice(d).col(zi), lam2(zi,d), S(k), d, P, Ip, nk, log2pi);
            } else {
              log_L -= likelihood_log_int(Hk, HHk, Yk, beta0, sig2, theta.slice(d).col(zj), lam2(zj,d), S(k), d, P, Ip, nk, log2pi);
            }
          }
        }
