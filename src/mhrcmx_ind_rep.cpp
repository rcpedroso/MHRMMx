...
double dmvnorm_log(arma::vec x, arma::vec m, double var, int P){
  arma::vec x_m = x - m;
  // double out = - 0.5*(double)P*log(var) - 0.5*arma::as_scalar(x_m.t()*x_m)/var;
...
////* hrcmx *////
// [[Rcpp::export]]
Rcpp::List mhrcmx_ind_JN(Rcpp::List Y,                  // observations
                  ...
                  const int burn=1e4, const int ns=1e3, const int thin=10,   // MCMC settings
                  const int H0=0,           // if H0=1 the model restrict sum(H*beta)=0
                  int J=0,                  // number of components in the FMMx
                  const double alpha_dir=1, // dirichlet prior
                  int repulsive=0,          // repulsive choice
                  double eps=1,             // proposal variance factor
  ...
  if (repulsive > 0) Rprintf("eps MH = %f\n", eps);
  Rprintf("\n");
  ...
  // ARWMH (Quinlan etal 2021)
  arma::cube Vadapt(P,P,J, arma::fill::zeros);
  arma::field<arma::cube> VadaptD(D);
  for (int d = 0; d < D; d++) VadaptD(d) = Vadapt;
  // MALA
  double eps2 = eps*eps;
  ...
  while (s < ns) {
    ...
    if (SM == 1) {
      ...
      while (t < nGS) {
        ...
        for (int d = 0; d < D; d++) {
          ...
          for (int l = 0; l < 2; l++) {
            ...
            lam2_s(l,d) = 1/rgamma_ars(A2, a_lam2_s, b_lam2_s);
          }
      ...
      while (t < nGS) {
        ...
        for (int d = 0; d < D; d++) {
          ...
          lam2_m(d) = 1/rgamma_ars(A2, a_lam2, b_lam2);
      ...
      if (zi == zj) {
        ...
        for (int d = 0; d < D; d++) {
          ...
          for (int l = 0; l < 2; l++) {
            ...
            lam2_s(l,d) = 1/rgamma_ars(A2, a_lam2, b_lam2);
        ...
        for (int d = 0; d < D; d++) {
          ...
          // lam2_m(d) = 1/rgamma_ars(A2, a_lam2, b_lam2); // HYPOTETICAL Gibbs sampling
        ...
        if (log(u) < log_alpha) {
          ...
          if (ppmx == 1) {
            ...
            if (covariates_cat == 1) {
              // ncatj.col(zij(0)) = ncatj_s.col(0);
              // ncatj.col(zj) = ncatj_s.col(1);
              for (int t = 0; t < Ncat; t++) {
      ...
      if (zi != zj) {
        ...
        for (int d = 0; d < D; d++) {
          ...
          for (int l = 0; l < 2; l++) {
            ...
            double b_lam2 = 0.5*beta_theta_clr_s(l);
            if (l == 0) {
              // lam2_s(l,d) = 1/rgamma_ars(A2, a_lam2, b_lam2); // HYPOTETICAL Gibbs sampling
          ...
          if (ppmx == 1) {
            ...
            if (covariates_cat == 1) {
              ...
              for (int t = 0; t < Ncat; t++) {
                ...
                Rprintf("cat%i ANTES\n", t);
                for (int i = 0; i < Ncat; i++) Rprintf("ncatJ(%i,%i,%i)=%i\n", i,zi,t,ncatJ(i,zi,t));
        ...
        if (nj(zi) == 0) {
          ...
          if (zi_sample >= nc ) {
            ...
            if (covariates_con == 1) {
              ...
              if (rnd(0)!=0.0 || rnd2(0)!=0.0) {
                // Rprintf("sumxj(zi)=%.20f\n", sumxj(zi));
                // Rprintf("sumx2j(zi)=%.20f\n", sumx2j(zi));
                // Rprintf("szi(0)=%.20f\n", szi(0));
                // Rprintf("s2zi(0)=%.20f\n", s2zi(0));
                Rprintf("sumxj(zi)=%f\n", sumxj(zi));
                Rprintf("sumx2j(zi)=%f\n", sumx2j(zi));
    ...
    for (int d = 0; d < D; d++) {
      ...
      update_theta(theta, Vadapt,
                   accept_mat, accept_logratio_mat,
                   nj, lam2, tau2, beta, mu, z, K, Ki,
                   m, d, J, P, Ip, n_theta, Ht0, theta_sampler,
                   repulsive, phi, nu, r, nr, eps, eps2,
                   b, burn);

      if (b <= burn) VadaptD(d) = Vadapt;
    ...
    for (int d = 0; d < D; d++) {
      ...
      for (int j = 0; j < J; j++) {
        ...
        if (nj(j)>0) {
          ...
          // ARS to sample from w = 1/x
          lam2(j,d) = 1/rgamma_ars(A2, a_lam2, b_lam2);
    ...
    for (int i = 0; i < m; i++) {
      ...
      for (int d = 0; d < D; d++) {
        ...
        arma::vec m_beta = V_beta * ( Hi.t()*(yi-beta0(i,d))/sig2(i,d) + theta.slice(d).col(zi)/lam2(zi,d) );
    ...
    for (int i = 0; i < m; i++) {
      ...
      for (int d = 0; d < D; d++) {
        ...
        if (Asig == 0) {
          ...
          // ARS to sample from w = 1/x
          sig2(i,d) = 1/rgamma_ars(A2sig, a_sig2, b_sig2);
  ...
  // ARWMH
  if (theta_sampler == 1) out["Vadapt"] = VadaptD;
