#ifndef RGAMMA_ARS_ADD_H
#define RGAMMA_ARS_ADD_H
...
/* ARS for truncated Gamma sampling */
// [[Rcpp::export]]
double rgamma_ars(const double L, const double Lu,
                  const double a, const double b) {
  
  double w;
  
  /* basic settings */
  // const double Lu = 1e10;
  ...
  while (accept == 0) {
    ...
    for (int j = 0; j < k; j++) {
      ...
      if (dhj > 0) {
        logA(j) = (a-1)*(log(xj)-1) + zu*dhj + log(1-exp(-(zu-zl)*dhj)) - log(dhj);
      } else {
    ...
    while (add == 0) {
      ...
      if (dhj > 0) {
        double ld = (zu-zl)*dhj;
        add = (ld + log(u*(1-exp(-ld))+exp(-ld)))/dhj;
        xstar = zl + add;
      } else {
        double ld = (zu-zl)*dhj;
        add = log(u*(exp(ld)-1)+1)/dhj;
        xstar = zl + add;
      }
    ...
    double r = (a-1) * ( log(xstar)-log(xj) + (xj-xstar)/xj );
