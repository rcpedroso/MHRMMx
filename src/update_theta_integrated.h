#ifndef UPDATE_THETA_INTEGRATED_H
#define UPDATE_THETA_INTEGRATED_H
...
void update_theta(arma::cube& theta,
  ...
  for (int i = 0; i < m; i++) {
    ...
    arma::mat Yi = Y[i];
    arma::vec yi = Yi.col(d) - beta0(i,d);
    arma::mat V = arma::inv( lam2(z(i),d)*HHi/sig2(i,d) + Ip );
    sumV.slice(z(i)) += V;
    sumVH.col(z(i)) += V*Hi.t()*yi/sig2(i,d);
  ...
  for (int j = 0; j < J; j++) {
    ...
    if (repulsive == 0) {
      ...
      if (nj(j) > 0) {
        ...
        theta.slice(d).col(j) = rmvnorm_eigen(m_j , V_j, lam2(j,d), P).t();
      ...
      if (theta_sampler == 1) { // ARWMH (Quinlan etal 2021)
        ...
        if (nj(j) > 0) {
          theta_prop = rmvnorm(1, theta_j , eps*Ki).t();
        } else {
          theta_prop = rmvnorm(1, theta_j , eps0*Ki).t();
        }
